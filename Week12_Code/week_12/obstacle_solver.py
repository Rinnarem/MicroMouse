...
#MANUAL_CORNERS = [(616, 112), (1501, 117), (1469, 994), (621, 968)]
MANUAL_CORNERS = None
...
def main():
    ...
    print("\n" + sep)
    print("PASTE INTO week_12.ino")
    print(sep)
    print("// Task 4.2 -- waypoints in GLOBAL maze millimetres.")
    print("// Frame: x = east (col), y = south (row); cell centre = (idx+0.5)*180.")
    print(f"// Generated from {IMAGE_FILE}. Section rows "
          f"{SECTION_TOP_ROW}-{SECTION_TOP_ROW+SECTION_SIZE-1}, cols "
          f"{SECTION_LEFT_COL}-{SECTION_LEFT_COL+SECTION_SIZE-1}.")
    print("const mtrn3100::Waypoint OBSTACLE_WAYPOINTS[] = {")
    for k, (x, y) in enumerate(gwp):
        tag = "entry" if k == 0 else ("exit" if k == len(gwp) - 1 else "")
        print(f"    {{ {x:7.1f}f, {y:7.1f}f }},"
              f"{'   // ' + tag if tag else ''}")
    print("};")
    print(f"#define OBSTACLE_WAYPOINT_COUNT  {len(gwp)}")
    er, ec = ENTRY_LOCAL
    xr, xc = EXIT_LOCAL
    print(f"#define OBS_ENTRY_ROW   {SECTION_TOP_ROW + er}"
          f"    // global maze row of the entry cell")
    print(f"#define OBS_ENTRY_COL   {SECTION_LEFT_COL + ec}"
          f"    // global maze col of the entry cell")
    print(f"#define OBS_ENTRY_DIR   mtrn3100::Maze::{DIR_NAME[ENTRY_DIR]}")
    print(f"#define OBS_EXIT_ROW    {SECTION_TOP_ROW + xr}")
    print(f"#define OBS_EXIT_COL    {SECTION_LEFT_COL + xc}")
    print(f"#define OBS_EXIT_DIR    mtrn3100::Maze::{DIR_NAME[EXIT_DIR]}")
    print(sep)
