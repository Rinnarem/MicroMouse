...
# ============================================================
MAZE_START = (7, 2, 'E')   # (row, col, heading) the demonstrator gives you
MAZE_GOAL  = (8, 5)        # (row, col)
...
def write_task42_full_header(pre_cmd, post_cmd, filename="gen_task42_full.h"):
    """Task 4.2 full-maze extras. The waypoints live in gen_task42.h, which the
    same run writes, so the two are always from the same photo."""
    import datetime
    lines = [
        "// ===================================================================",
        ...
        f"// start      : ({MAZE_START[0]}, {MAZE_START[1]}) facing "
        f"{os_.DIR_NAME[MAZE_START[2]]}",
        f"// goal       : ({MAZE_GOAL[0]}, {MAZE_GOAL[1]})",
        "//",
        "// The waypoints for the middle stage are in gen_task42.h, written by",
        "// the same run of this script.",
        "#pragma once",
        "",
        f'#define GEN_TASK42FULL_IMAGE "{IMAGE_FILE}"',
        "",
        f'#define PRE_OBSTACLE_PATH   "{pre_cmd}"',
        f'#define POST_OBSTACLE_PATH  "{post_cmd}"',
        f"#define MAZE_START_ROW      {MAZE_START[0]}",
        f"#define MAZE_START_COL      {MAZE_START[1]}",
        f"#define MAZE_START_DIR      mtrn3100::Maze::{os_.DIR_NAME[MAZE_START[2]]}",
        "",
    ]
...
def main():
    ...
    # ---------------- output ----------------
    # Both headers are written by THIS run, from THIS photo, so the waypoints
    # and the pre/post paths physically cannot come from different images --
    # which is the one mistake that would drive the robot into a wall.
    os_.write_task42_header(gwp, margin)
    write_task42_full_header(pre_cmd, post_cmd)

    print("\n" + sep)
    print("  gen_task42.h and gen_task42_full.h written. NOTHING TO COPY.")
    print(sep)
    print(f"  Set TASK_NUM to 4 in week_12.ino, then upload.")
    print(f"  Place the robot in cell ({MAZE_START[0]}, {MAZE_START[1]}) facing "
          f"{os_.DIR_NAME[MAZE_START[2]]}.")
    print(f"  It should finish in cell ({MAZE_GOAL[0]}, {MAZE_GOAL[1]}).")
    print(sep)
